...
dfs :: (ProblemState s a, Ord s) => s  -> Int -> [Node s a]
dfs start dp  = helper [Node start [] 0] [] dp where
				helper [] visited dp = visited
				helper ((nod@(Node lvl pos rang)) : xs) visited dp
					| dp == rang = (helper xs (nod:visited) dp)
					| any (== lvl) (map nodeState visited) = helper xs visited dp
					| otherwise = helper (vecini ++ xs ) (nod:visited) dp where
						vecini = (map (\l -> Node (extractSt l) (extractPos l) (rang+1))(successors lvl))
...
limitedDfs start bonus deep = reverse (dfs start deep) 
